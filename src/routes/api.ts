			...
			proxy_check(proxy)
				.then((r: boolean) => {
					...
					if (password) {
						...
						const runCommand = `sudo docker run -it --cpus=2 -m 1024m --memory-reservation=512m --net=bridge --restart always -p ${proxy.port}:${proxy.port} --name mitmproxy${proxy.port} -d mitmproxy/mitmproxy:latest mitmproxy --set block_global=false --set listen_port=${proxy.port} --mode upstream:http://${proxy.host}:${proxy.port}`;
